...
export class TicketSupportBackendStack extends Stack {
  constructor(scope: Construct, id: string, props: TicketSupportBackendStackProps) {
    const { deploymentConfig, lambdaSourceDirectory: sourceOverride, ...stackProps } = props;
    ...
    const jobApiUrl = jobApi.addFunctionUrl({
      ...
      cors: {
        ...
        allowedOrigins: [deploymentConfig.corsAllowedOrigin],
        maxAge: Duration.minutes(5),
      },
    });

    const supervisor = new lambdaNodejs.NodejsFunction(this, 'SupervisorFunction', {
      ...commonFunctionProps,
      entry: path.join(lambdaSourceDirectory, 'supervisor.ts'),
      handler: 'handler',
      description: 'Runs the bounded Strands supervisor for queued ticket jobs',
      environment: {
        ...commonEnvironment,
